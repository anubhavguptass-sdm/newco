...
(function ($) {
  ...
  VideoGallery.prototype = {
    ...
    bindEvents: function bindEvents() {
      ...
      this.$arrows.click("click", function () {
        ...
        var $this = $(this),
            ...
            maxLeft = listOuterWidth - self.listContainerWidth + self.listContainerWidth % self.thumbnailWidth;
        self.$arrows.removeClass("is-disabled");

        if (nextLeft <= 0 && -1 * nextLeft <= maxLeft) {
          self.$thumbnailsList.animate({
            left: nextLeft
          });
        } else {
    ...
    reloadVideo: function reloadVideo(data) {
      ...
      if (mediaElement.pluginType === "youtube") {
        mediaElement.pluginApi.loadVideoById(data.src[0].src.split("?v=")[1]);
      } else if (data.src.length) {
        // CF-608 - newer MediaElement doesnt accept undefined here
